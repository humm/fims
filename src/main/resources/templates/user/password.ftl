...
<script>
    layui.config({
    ...
    }).use(['index', 'admin', 'fims', 'set'], function () {
        var $ = layui.$,
            form = layui.form,
            admin = layui.admin,
            fims = layui.fims;
        ...
        $(document).on('click', '.layui-btn', function () {
            ...
            if(fims.isBlank(password) || password.length < 6 || password.length > 16 || password != repassword){
                return;
            }
            if(oldPassword != userPassword){
                fims.msg(fims.tips.msg.historyPasswordError);
                return;
            }
            if(password == userPassword){
                fims.msg(fims.tips.msg.historyPasswordEqualPassword);
                return;
            }
            var request = {
